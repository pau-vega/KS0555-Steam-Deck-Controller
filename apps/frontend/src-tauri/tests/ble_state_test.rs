...
mod tests {
    ...
    fn test_ble_state_struct_exists() {
        ...
        let content =
            fs::read_to_string("src/ble/state.rs").expect("Should be able to read state.rs");

        assert!(
            content.contains("pub struct BleState"),
            "BleState struct should be public"
        );
        assert!(
            content.contains("Arc<Mutex<Option<Peripheral>>>"),
            "Should use Arc<Mutex<Option<Peripheral>>>"
        );
    }
    ...
    fn test_ble_state_has_new_method() {
        ...
        assert!(
            ...
            "BleState should have new() constructor"
        );
    }
    ...
    fn test_ble_state_has_set_method() {
        ...
        assert!(
            content.contains("pub fn set(&self, peripheral: Option<Peripheral>)"),
    ...
    fn test_ble_state_has_get_method() {
        ...
        assert!(
            content.contains("pub fn get(&self) -> Option<Peripheral>"),
    ...
    fn test_ble_state_implements_clone() {
        ...
        assert!(
            content.contains("impl Clone for BleState"),
    ...
    fn test_ble_state_uses_arc_mutex() {
        ...
        assert!(
            content.contains("use std::sync::{Arc, Mutex}"),
            "Should import Arc and Mutex"
        );
        assert!(
            content.contains("pub peripheral: Arc<Mutex<Option<Peripheral>>>"),
