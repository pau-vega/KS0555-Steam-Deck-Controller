...
mod flatpak_sandbox_tests {
    ...
    fn test_in_flatpak_function_exists() {
        let content = fs::read_to_string("src/lib.rs").expect("Should be able to read lib.rs");
        assert!(
            content.contains("fn in_flatpak"),
            "lib.rs must define fn in_flatpak()"
        );
        assert!(
            content.contains("FLATPAK_ID"),
            "in_flatpak() must check FLATPAK_ID env var"
        );
        assert!(
            content.contains("/.flatpak-info"),
    ...
    fn test_webkit_set_var_remains_unconditional() {
        let content = fs::read_to_string("src/lib.rs").expect("Should be able to read lib.rs");
        assert!(
            content.contains("WEBKIT_DISABLE_COMPOSITING_MODE"),
        ...
        for (i, c) in after_gate.chars().enumerate() {
            ...
            if in_block && depth == 0 {
                gate_close = gate_start + i;
                break;
            }
        }
        assert!(
            gate_close > 0,
            "Should find closing brace for !in_flatpak block"
        );
        let webkit_pos = content
    ...
    fn test_lib_rs_has_no_flatpak_gate_outside_db_block() {
        ...
        let content = fs::read_to_string("src/lib.rs").expect("Should be able to read lib.rs");
        assert!(
            content.contains("tauri::Builder"),
    ...
    fn test_manifest_has_ble_finish_args() {
        ...
        assert!(
            content.contains("--system-talk-name=org.bluez"),
    ...
    fn test_manifest_has_no_anti_features_in_active_finish_args() {
        ...
        assert!(
            !finish_args_section.contains("--filesystem=home"),
            "Active finish-args must not contain --filesystem=home"
        );
        assert!(
            !finish_args_section.contains("--socket=session-bus"),
            "Active finish-args must not contain --socket=session-bus"
        );
        assert!(
            !finish_args_section.contains("--socket=system-bus"),
    ...
    fn test_manifest_uses_system_talk_name_not_session() {
        ...
        let finish_args_section = &content[finish_args_start..];
        assert!(
            finish_args_section.contains("--system-talk-name"),
    ...
    fn test_manifest_structure_intact() {
        ...
        assert!(
            content.contains("build-commands:"),
