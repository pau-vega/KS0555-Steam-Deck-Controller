...
mod tests {
    ...
    fn test_post_filter_handles_exact_match() {
        ...
        assert!(
            content.contains("name == BT24_NAME"),
    ...
    fn test_linux_bluez_pitfall_commented() {
        ...
        assert!(
            content.contains("Linux/BlueZ"),
            "Should mention Linux/BlueZ issue"
        );
        assert!(content.contains("Pitfall 2"), "Should reference Pitfall 2");
        assert!(
            content.contains("merges discovery filters"),
    ...
    fn test_service_uuid_verification_optional() {
        ...
        assert!(
            content.contains("service-UUID verification"),
            "Should have service UUID verification comment"
        );
        assert!(
            content.contains("0000ffe0"),
    ...
    fn test_post_filter_in_ble_connect() {
        ...
        let start = content
            .find("pub async fn ble_connect")
            .expect("ble_connect should exist");
        let end = content
            .find("pub fn setup_event_listener")
            .unwrap_or(content.len());
        let connect_fn = &content[start..end];

        assert!(
            connect_fn.contains("name.contains(BT24_NAME)"),
