...
describe("StatusBar", () => {
  ...
  it('shows "Bluetooth" label', () => {
    const { container } = render(_jsx(StatusBar, { bleConnected: true, gamepadConnected: false }))
  ...
  it("shows X when Bluetooth disconnected", () => {
    const { container } = render(_jsx(StatusBar, { bleConnected: false, gamepadConnected: false }))
    const bluetoothPill = container.firstElementChild?.firstElementChild
    expect(bluetoothPill?.textContent).toContain("✗")
    expect(bluetoothPill?.textContent).toContain("Bluetooth")
  })
  it('shows "Connecting..." when connecting', () => {
    const { container } = render(_jsx(StatusBar, { bleConnected: false, gamepadConnected: false, connecting: true }))
    const bluetoothPill = container.firstElementChild?.firstElementChild
    expect(bluetoothPill?.textContent).toContain("Connecting...")
  })
  it("shows Gamepad connected state", () => {
    const { container } = render(_jsx(StatusBar, { bleConnected: true, gamepadConnected: true }))
